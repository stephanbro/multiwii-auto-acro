...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  case 1:
  {
      ...
      debug[1] = v;
        ...
        ind %= VBAT_SMOOTH;
        #if VBAT_SMOOTH == 16
          analog.vbat = vsum / conf.vbatscale + 2; // result is Vbatt in 0.1V steps
        #elif VBAT_SMOOTH < 16
...
void go_disarm() {
  ...
  static uint8_t  targetAltReached = 0;
  static uint16_t recoverTime = 0;
  static uint8_t  auton_switch = INIT; //Current state for autonomous mode
  static uint8_t  hasFlipped = 0;       //Flipped or not
  static uint8_t  flipCount = 0;       //Number of flips
  static uint16_t holdTimer = 0;       //Timer to hold altitute before descending
  static uint8_t  targetAltReachedCounter = 0; //Number of times target alt must be reached to go to next state
  static int32_t  holdStateAlt = 0;     //Hold altitude

  static uint8_t auton_flag = 0;  
  void resetAltHold(int32_t targetAlt) {
     errorAltitudeI = 0;         // clear all ALT_HOLD code values to default of OFF
     BaroPID = 0;
     AltHold = targetAlt;
     targetVario = 0;
     targetAltReached = 0;
     rcCommand[THROTTLE] = THROTTLE_HOVER;
     rcData[THROTTLE] = THROTTLE_HOVER;
  }
  
  void resetAutonState() {
    rcOptions[BOXBARO]=0;    //Turn off Altitude Hold
    rcOptions[BOXANGLE]=0;   //Turn off Auto Level
    rcOptions[BOXMAG]=0;     //Turn off Heading Hold
    rcOptions[BOXGPSHOLD]=0; //Turn off GPS Hold Mode
    flipCount = 0;          //Reset number of flips
    hasFlipped = 0;          //Reset flip state
    holdTimer = 0;           //Reset holdTimer for altitude hold
    targetAltReachedCounter = 0;  //Reset number of times reached target
    auton_switch = INIT;	 //Reset auton_switch to INIT mode
    recoverTime = 0;		 //Reset recover timer
    AltHold = 0;			 //Reset AltHold to the grouping
    errorAltitudeI = 0;		 //Reset altitude error
    BaroPID = 0;			 //Reset BaroPID
    auton_flag = 0;			 //Turn off auton mode
  }
  
  int altToTarget(uint16_t altitude, uint8_t vario) {
    altVarioTime = currentTime + PROC_SPEED/vario;
    if (alt.EstAlt > (altitude + ALT_ERROR_DEADBAND)) {                
      AltHold--;
      targetVario = -vario;
      return 0;
    } else if (alt.EstAlt < (altitude - ALT_ERROR_DEADBAND)) {
      AltHold++;
      targetVario = vario;
      return 0;
    } else {
      AltHold = holdStateAlt;
      errorAltitudeI = 0;
      targetVario = 0;
      return 1;
    }
  ...
  void autoLand(uint8_t fastVario, uint8_t slowVario, uint16_t safetyAlt) {
    if(alt.EstAlt > safetyAlt) {
      altVarioTime = currentTime + PROC_SPEED/fastVario;
      targetVario = -fastVario;
    } else {
      altVarioTime = currentTime + PROC_SPEED/slowVario;
      targetVario = -slowVario;
    }
    AltHold--;
  }
  
  void rc_options(uint8_t state, uint8_t auton) {
    uint16_t auxState = 0;
    uint8_t i;
    if(auton) {
      if(state==CLIMB||state==HOLD||state==LAND||state==FLIP_RECOVER) {
        if((state == HOLD) &&(GPS_distanceToHome>TRAVEL_HOME_LATCH)) {
          rcOptions[BOXGPSHOME] = 1;
        } else if(rcOptions[BOXGPSHOME] &&!(GPS_distanceToHome<TRAVEL_HOME_UNLATCH)) {
          rcOptions[BOXGPSHOME] = 1;
        } else {
          rcOptions[BOXGPSHOME] = 0;
        }
        rcOptions[BOXBARO]=1;    //Altitude Hold Mode
        rcOptions[BOXGPSHOLD]=1; //GPS Hold Mode
        rcOptions[BOXANGLE]=1;   //Level Flight Mode
        rcOptions[BOXMAG]=1;     //Heading Hold Mode
      } else if(state==FLIP_CLIMB) {
        rcOptions[BOXBARO]=0;
      ...
      if(rcOptions[BOXGPSHOLD]&&GPS_distanceToHome>TRAVEL_HOME_LATCH&&state!=LAND) {
        rcOptions[BOXGPSHOME] = 1;
      } else if(rcOptions[BOXGPSHOME]&&!(GPS_distanceToHome<TRAVEL_HOME_UNLATCH)) {
        rcOptions[BOXGPSHOME] = 1;
      } else {
      ...
      if(state != FLIP) {
        rcData[ROLL] = THROTTLE_MID;   //Mid-Roll
        rcData[PITCH] = THROTTLE_MID;  //Mid-Pitch
        rcData[YAW] = THROTTLE_MID;    //Mid-Yaw
      }    
      rcData[THROTTLE] = rcCommand[THROTTLE]; //rcCommand[THROTTLE] written to by BaroPID and rcData, used to ignore transmitter
    } else {
      for(i=0;i<4;i++)
  ...
  #ifdef SJB_AUTON_TEST
  void auton_test() {
    static uint16_t autonTimer = 0;
    static uint8_t  landing_test = 0;
    auton_flag = 1;        //Force auton_flag on
    rcData[AUX1] = 1902;   //Force AUX1>1900 to stay armed
    if((landing_test == 1)&&(auton_switch == INIT)) {	//Completed Test Cases
      landing_test = 0;
      auton_switch = 20;	//All tested state
    } else if(auton_switch == INIT) {
      if(autonTimer > 500) {  //Wait for 10 seconds to allow a reconnect on the GUI, then set variables
        holdStateAlt = alt.EstAlt + HOLD_STATE_ALT;
        rcCommand[THROTTLE] = THROTTLE_HOVER; //Inititalize throttle at mid-range
        rcData[THROTTLE] = THROTTLE_HOVER;
        targetAltReachedCounter = 0;
        go_arm();
        autonTimer = 0;
        auton_switch = CLIMB;
      } else {
    ...
    } else if(auton_switch == CLIMB) {
      if(AltHold>holdStateAlt) {	//AltHold has reached our target
        if((abs(autonTimer - (holdStateAlt*50)/CLIMB_VARIO)<100)&&(BaroPID>250)&&(rcCommand[THROTTLE]>(THROTTLE_HOVER+200))) {
          holdTimer = 0;
          autonTimer = 0;
          auton_switch=HOLD; //Leave CLIMB state when we pass defined Alt
        }
      } else {
        autonTimer++;
      }
      debug[0] = AltHold;
      debug[1] = autonTimer;
    } else if(auton_switch == HOLD) {
      holdTimer = HOLD_TIME_CYCLES/20 + 1;
      rcData[AUX1] = 1971;	//Force AUX1 high to move to next state
    } else if(auton_switch == FLIP_CLIMB) {
      auton_switch = FLIP;
    } else if(auton_switch == FLIP) {
      //Add test case for this state
    } else if(auton_switch == FLIP_RECOVER) {
      if(AltHold == holdStateAlt) {
        hasFlipped = 1; //We've now "flipped", used to go to land state from Hold
        holdTimer = 0;
        auton_switch = HOLD;
      }
    } else if(auton_switch == LAND) {
      landing_test = 1;
    } else if(auton_switch == 20) {
      auton_switch = 20;
    } else {
      auton_switch = 13;
    }
...
void loop () {
  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    #ifdef SJB_AUTON      
      //Test Cases
      #if defined(SJB_AUTON_TEST)  //Remove #ifdef in multiwii.h when field testing, set up battery in the future
        auton_test();
      #endif
      /*
      if(auton_switch == INIT) {
        auton_flag = 1;
        auton_switch = FLIP;
      }
      rcData[AUX1] = 1920;
      */
      if (auton_flag == 1 && rcData[AUX1] <= 1901) {	//Auton mode disabled from AUX1 switch
        resetAutonState();
      }

      //Autonomous Mode Logic
      if(auton_flag) {
        switch (auton_switch) {    //Setup Autonomous States
          case INIT:      //initial state used to wait for Sat Lock before using GPS Position Hold
            rcCommand[THROTTLE] = THROTTLE_OFF;
            if(f.GPS_FIX && GPS_numSat >= 5) {
              holdStateAlt = alt.EstAlt + HOLD_STATE_ALT;
              rcCommand[THROTTLE] = THROTTLE_HOVER; //Inititalize throttle at mid-range
              initialThrottleHold = THROTTLE_HOVER;
              targetAltReachedCounter = 0;
              go_arm();
              auton_switch = CLIMB;
            }
          break;
          case CLIMB:              //Climb State
            goalAlt = holdStateAlt;
            varioRate = CLIMB_VARIO;
            if(targetAltReached) {
              altToTargetEnable = 0;
              resetAltHold(goalAlt);
              //targetAltReachedCounter++;
              //if(targetAltReachedCounter > ALT_REACHED_CYCLES) {
                holdTimer = 0;
                auton_switch=HOLD; //Leave CLIMB state when we pass defined Alt
              //}
            } else {
          ...
          break;
          case HOLD:          //Hold State, Used as intermediary state
            if(holdTimer > HOLD_TIME_CYCLES/20&&!rcOptions[BOXGPSHOME]) {
              if(rcData[AUX1] > STATE_SWITCH) {
                if(hasFlipped&&(flipCount>=MAX_FLIP_COUNT)) {  //Have we flipped or not?
                  auton_switch = LAND;       //Land after we've flipped
                } else {
                  hasFlipped = 0;
                  recoverTime = 0;
                  auton_switch = FLIP_CLIMB; //Climb before we flip
                }
              }
            } else if(abs(alt.EstAlt - holdStateAlt)<ALT_DEADBAND) {
              holdTimer++;
            } else {
              holdTimer = 0;
            }
          break;
          case FLIP_CLIMB:            //Climb before aa flip
            rcCommand[THROTTLE] = THROTTLE_FULL;  //Full Throttle
            if(alt.EstAlt >= holdStateAlt + FLIP_ALT_BUFFER) auton_switch = FLIP; //Climb tiil we reach our alt goal, FLIP_FALL becomes FLIP later
          break;
          case FLIP:    //Flip state, needs to wait for other testing
            rcData[PITCH] = THROTTLE_MID;  //Mid-Pitch
            rcData[YAW] = THROTTLE_MID;    //Mid-Yaw
            if(flipCount == 0) {
              rcData[ROLL] = THROTTLE_FULL;   //Mid-Roll
            } else if(flipCount == 1) {
              rcData[ROLL] = THROTTLE_OFF;
            }
            if(abs(att.angle[ROLL]) < FLIP_ANGLE_CUTOUT*10) { //Full Power 1200 = 120.0 degrees
              rcCommand[THROTTLE] = THROTTLE_FULL;
            } else {                      //Kill Power when Upside-down
              rcCommand[THROTTLE] = THROTTLE_LOW;
              if(!hasFlipped) {
                hasFlipped = 1;
              }
            }
            if(hasFlipped&&(abs(att.angle[ROLL])<FLIP_ANGLE_HOLD*10)) {//&&(att.angle[ROLL]<0)) {  //Go back to hold after reaching threshold
              recoverTime = 0;
              flipCount ++;  //Increment flip count after cutting the throttle
              auton_switch = FLIP_RECOVER;
            }
          break;
          case FLIP_RECOVER:
            if((recoverTime >= RECOVER_CYCLES/20)||(alt.EstAlt >= holdStateAlt + FLIP_ALT_BUFFER)) {
              rcCommand[THROTTLE] = THROTTLE_HOVER;
              initialThrottleHold = THROTTLE_HOVER;
              goalAlt = holdStateAlt;
              varioRate = CLIMB_VARIO;
              ...
              rcCommand[THROTTLE] = THROTTLE_FULL;
              AltHold = holdStateAlt + FLIP_ALT_BUFFER;
              recoverTime++;
            }
          break;
          case LAND:  //Land State
            if((alt.EstAlt<AUTOLAND_SAFETY_ALT)&&(BaroPID<BARO_PID_CUTOFF)) {
              autoLandEnable = 0;
              go_disarm();
              resetAutonState();
            } else {
      ...
      rc_options(auton_switch, auton_flag);
    #endif
    #ifndef SJB_AUTON_TEST
      debug[0] = analog.vbat;
      //debug[1] = BaroPID;
    #endif
      debug[2] = rcCommand[THROTTLE];
      debug[3] = auton_switch;
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
        ...
        #endif
        #ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
          else if (conf.activate[BOXARM] == 0 && rcSticks == THR_LO + YAW_CE + PIT_CE + ROL_HI && rcData[AUX1] <= 1901) go_arm();      // Arm via ROLL
        #endif
        #ifdef SJB_AUTON
          else if (conf.activate[BOXARM] == 0 && rcSticks == THR_LO + YAW_CE + PIT_CE + ROL_HI && rcData[AUX1] > 1901) {
            resetAutonState();
            auton_flag = 1;
          }
        #endif
    ...
    if (rcOptions[BOXARM] == 0) f.OK_TO_ARM = 1;
      ...
      if (f.GPS_FIX && GPS_numSat >= 5 ) {
        if (rcOptions[BOXGPSHOME]) {  // if both GPS_HOME & GPS_HOLD are checked => GPS_HOME is the priority
          ...
          f.GPS_HOME_MODE = 0;
          if (rcOptions[BOXGPSHOLD] && (auton_flag||(abs(rcCommand[ROLL])< AP_MODE && abs(rcCommand[PITCH]) < AP_MODE))) {
            if (!f.GPS_HOLD_MODE) {
              f.GPS_HOLD_MODE = 1;
              GPSNavReset = 0;
              #if defined(I2C_GPS)
...
*/
 ...
 //*********************************** 
  ...
  #if defined(SJB_AUTON)
    if(altToTargetEnable||autoLandEnable) {
      if(currentTime > altVarioTime) {            // basic calculation for different alt hold modes and failsafe modes
        if(altToTargetEnable) {
          targetAltReached = altToTarget(goalAlt, varioRate);
        } else if(autoLandEnable) {
          autoLand(AUTOLAND_FAST_VARIO, AUTOLAND_SLOW_VARIO, AUTOLAND_SAFETY_ALT);
        }
      }
    }    
  #endif
    ...
    if (f.BARO_MODE) {
      ...
      if (abs(rcCommand[THROTTLE]-initialThrottleHold)>ALT_HOLD_THROTTLE_NEUTRAL_ZONE  && !auton_flag) {
